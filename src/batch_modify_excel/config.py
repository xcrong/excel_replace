class Config:
    ...
    # 默认清理字符
    DEFAULT_CLEAN_CHAR = '（ 免费）'
    # 分隔符列表
    SEPARATORS = [',', '，', ' ', '、', ', ']
    # 项目信息
    PROJECT_INFO = """Excel 批量清理工具
版本：0.1.0
这是一个用于批量清理Excel文件的工具。它可以帮助您快速地清理Excel文件中的指定字符，提高工作效率。"""
    # 作者信息
    AUTHOR_INFO = """作者：xcrong
邮箱：zlz_gty@foxmail.com
如果您有任何问题或建议，请随时与我联系。"""
    # 使用说明
    INSTRUCTIONS = """1. 选择要清理的Excel文件或文件夹
2. 输入要清理的列和行（可用英文逗号、中文逗号、顿号或空格分隔）
3. 输入要清理的字符
4. 点击'执行清理'按钮开始处理
5. 根据需要，选择是否覆盖原文件
6. 等待处理完成，查看处理结果"""
    # 窗口标题
    WINDOW_TITLE = "Excel 批量清理工具"
    # 关于菜单标签
    ABOUT_MENU_LABEL = "关于"
    # 项目信息标签
